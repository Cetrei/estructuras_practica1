...
\newline
\textbf{Complejidad espacial:} $E(1)$. La función solo reserva variables locales de tamaño fijo.
...
\newline
\textbf{Complejidad espacial:} $E(n)$. Se reserva un arreglo de $n$ elementos de tipo \texttt{Historial}, cuyo tamaño crece proporcionalmente a la cantidad de registros
...
\newline
\textbf{Complejidad espacial:} $E(1)$. La función solo usa variables locales de tamaño fijo
...
\newline
\textbf{Complejidad espacial:} $E(1)$. La función solo mantiene un registro \texttt{Estudiante} a la vez en memoria
...
\newline
\textbf{Complejidad espacial:} $E(n)$. Está dominada por el arreglo dinámico de historiales reservado en \texttt{cargarHistorial()}, ya que el resto de las funciones usa memoria constante.
...
\midrule
\texttt{contarRegistros()} & $O(4)$ = $O(1)$ & $E(1)$ \\
\texttt{cargarHistorial()} & $O(n + 3)$ = $O(n)$ & $E(n)$ \\
\texttt{calcularEstadisticas()} & $O(n)$ & $E(1)$ \\
\texttt{buscarEstudiante()} (nota menor) & $O(m + 2)$ = $O(m)$& $E(1)$ \\
\texttt{buscarEstudiante()} (nota mayor) & $O(m + 2)$ = $O(m)$ & $E(1)$ \\
\texttt{construirReporte()} & $O(1)$ & $E(1)$ \\
\texttt{imprimirReporte()} & $O(1)$ & $E(1)$ \\
\midrule
\textbf{Programa completo} & $O(n + m)$ & $E(n)$ \\
\textbf{Programa completo (real)} & $O(2n + 2m + 14)$ & $E(n + 6)$ \\
\bottomrule
\end{tabular}
\end{center}
